...
export const produceSalesSummary1 = async (schedules, branch) => {
  ...
  const depositList = await DepositsClass.getDataByFieldName(
  ...
  const depositDList = await SchedulersClass.getDataByPartialDate(
  ...
  if (depositDList.length > 0) {
    ...
    const _data = {
      [SchedulersClass.DATE_ORDER_PLACED]: formatDateDash(dateTobeFilterCopy),
      [SchedulersClass.DATE_START]: formatDateDash(dateTobeFilterCopy),
      [SchedulersClass.DATE_PAYMENT]: formatDateDash(dateTobeFilterCopy),
      [SchedulersClass.UTAK_NO]: "",
      [SchedulersClass.PARTNER_MERCHANT_ORDER_NO]: "",
      [SchedulersClass.ZAP_NUMBER]: "",
      [SchedulersClass.MODE_PAYMENT]: "Cash",
